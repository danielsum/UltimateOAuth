...
●index.html(ユーザーにクリックさせるリンクがあるページ)
<a href="req.php">認証</a>
...
また、アクセストークンからオブジェクトを生成したい場合は、twitteroauthのように
<?php
...
★サンプルいろいろ
  ...
  ではなく
    is_array($res)
  を使う必要がある。(配列に対してアロー演算（演算子は「->」）を行うとその時点でエラーが発生するため)
  */
  if (is_array($res)) {
  ...
  ●同一ディレクトリにあるtest.pngを添付してツイート(エラーチェック省略)
  <?php
  # $uoに認証済みのUltimateOAuthオブジェクトがセットされた状態で
...
★UltimateOAuthRotateクラスを使う
    自動的にGETリクエストに関してのAPI規制回避を行えるクラスで、UltimateOAuthオブジェクトとほぼ同様に扱える
<?php
...
*/
$uor->register('コンシューマーキー','コンシューマーシークレット','スクリーンネーム','パスワード');
